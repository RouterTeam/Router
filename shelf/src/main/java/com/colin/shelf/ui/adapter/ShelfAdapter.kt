...
import com.colin.shelf.databinding.ItemShelfSerialTopLayoutBinding
import com.colin.shelf.databinding.ItemShelfSerialTopLayoutBindingImpl
...
class ShelfAdapter(context: Context?) : BaseBindAdapter<Any, ViewDataBinding>(context) {
    ...
    override fun getItemType(position: Int): Int {
        ...
        if (position == 0 && item is SerialStory) return 3
    ...
    override fun getItemLayout(viewType: Int): Int {
        return when (viewType) {
            ...
            3 -> R.layout.item_shelf_serial_top_layout
    ...
    override fun onBindItem(binding: ViewDataBinding?, item: Any?, position: Int) {
        when (binding) {
            ...
            is ItemShelfSerialTopLayoutBinding ->{
                binding?.serialStory = item as SerialStory
            }
        }
    }
    ...
    override fun getCustomerSpanSize(position: Int): Int {
        val item = datas[position]
        if (position == 0 && item is SerialStory) return 2
